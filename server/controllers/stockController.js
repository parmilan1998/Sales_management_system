    ...
    try {
        ...
        const stocksItems = await Promise.all(
            stocks.map(async (stock) => {
                const {productName, productQuantity, purchasePrice, manufacturedDate, expiryDate, purchasedDate} =
                    stock;

                try {
                    if (!productName || !productQuantity || !purchasePrice || !manufacturedDate || !expiryDate) {
                    ...
                    if (existingStock) {
                        const updatedStock = await existingStock.update({
                            productQuantity: (existingStock.productQuantity += parseInt(productQuantity)),
                        });
                        return updatedStock;
                    } else {
...
exports.updateStocks = async (req, res) => {
    try {
        const {id} = req.params;
        const {productName, productQuantity, purchasePrice, manufacturedDate, expiryDate, purchasedDate} = req.body;
